...
from find_port import find_process_port
...
class ArterySynchronization(object):
    ...
    def getCarlaLocationDENM(self,synchronization,cam):
        x,y = synchronization.net.convertLonLat2XY(cam['Longitude'],cam['Latitude'])
        extent    = carla.Vector3D(1023.0 / 400.0, 62.0 / 400.0, 0/ 2.0)
        transform = carla.Transform(carla.Location(x, y, 0),
                                carla.Rotation())
        sender_carla_transform = BridgeHelper.get_carla_transform(transform, extent)
        cam['sender_pos_x']=sender_carla_transform.location.x
    ...
    def camToDict(self,synchronization,cam):
        ...
        full_cam['Longitudinal Acceleration']=float(full_cam['Longitudinal Acceleration [Confidence]'].split('[')[0])
        full_cam['Longitudinal Acceleration Confidence']=float(full_cam['Longitudinal Acceleration [Confidence]'].split('[')[1].split(']')[0])
    ...
    def denmToDict(self,synchronization,denm):
        full_split=[x.split(':') for x in denm.split('\n')]
        full_denm= {k:v for k,v in full_split[:-1]}
        # full_denm['ITS PDU Header']=str(full_denm['ITS PDU Header'])
        full_denm['Protocol Version']=int(full_denm['Protocol Version'])
        full_denm['Message ID']=int(full_denm['Message ID'])
        full_denm['Station ID']=int(full_denm['Station ID'])

        full_denm["Originating Station ID"]=int(full_denm["Originating Station ID"])
        full_denm["Sequence Number"]=int(full_denm["Sequence Number"])
        full_denm["Alitude [Confidence]"]=float(full_denm['Altitude [Confidence]'].split('[')[1].split(']')[0])
        full_denm["Longitude"]=float(full_denm["Longitude"])/10**7
        full_denm["Latitude"]=float(full_denm["Latitude"])/10**7
        full_denm["Semi Major Orientation"]=int(full_denm['Semi Major Orientation'])
        full_denm["Semi Major Confidence"]=int(full_denm['Semi Major Confidence'])
        full_denm["Semi Minor Confidence"]=int(full_denm['Semi Minor Confidence'])
        full_denm["Relevance Distance"]=int(full_denm["Relevance Distance"]) # Number Corresponds to something
        full_denm["Relevance Traffic Detection"]=int(full_denm["Relevance Traffic Detection"])
        full_denm["Validity Duration"]=int(full_denm["Validity Duration"])

        full_denm["Speed"]=float(full_denm["Speed"])
        full_denm['Speed_Confidence']=float(full_denm['Speed Confidence'])
        full_denm['Heading']=float(full_denm['Heading [Confidence]'].split('[')[0])
        full_denm['Heading_Confidence']=float(full_denm['Heading [Confidence]'].split('[')[1].split(']')[0])

        full_denm["Information Quality"]=int(full_denm["Information Quality"])
        full_denm["Cause Code"]=int(full_denm["Cause Code"])
        full_denm["Sub Cause Code"]=int(full_denm["Sub Cause Code"])

        full_denm.pop('Situation')
        full_denm.pop('Location')
        full_denm.pop('Management')
        full_denm.pop('ITS PDU Header')

        self.getCarlaLocationDENM(synchronization,full_denm)
    ...
    def recieve_denm_messages(self,synchronization):
        current_step_denms=[]
        count = 0
        while True:
            data = None
            try :
                ...
                if self.on_going_message_recv['state']:
                    if  self.get_ongoing_recv():
                        ...
                        data = self.on_going_message_recv['fragment']
                        if self.on_going_message_recv['full_message_size'] ==6 :
                            next_denm_size = int(data.decode('utf-8'))
                            data = self.conn.recv(next_denm_size)
                            if self.set_ongoing_recv(data,next_denm_size):
                                continue
                else :
                    data = self.conn.recv(6)
                    if len(data)<=0:
                ...
                if e.errno != errno.EWOULDBLOCK: 
                    ...
                    # print(data)
                break
            full_denm = self.denmToDict(synchronization,data.decode('utf-8'))
            current_step_denms.append(full_denm)
        print(current_step_denms)
        return current_step_denms
