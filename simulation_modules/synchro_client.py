...
sys.path.append('../CarlaSumoArtery-CoSimulation/Folan_Modules')
...
from folan_attack_module import PositionAttacker, TrafficJamAttack, ConstantOffsetAttacker, RandomOffsetAttacker
from folan_attack_module import TractionLossAttack, EmergencyBrakingAttack, TrafficJamAttack
...
class Simulation():
    def __init__(self,args,rl_model=None):
        ...
        victim_blueprint = self.blueprint_library.filter('vehicle.mercedes.sprinter')[0]

        self.map = self.world.get_map()
        self.waypoints = self.map.generate_waypoints(5) # 4 meters apart?

        spawn_point = self.waypoints[1500].transform
        spawn_point.location.z += 2
        # transform = carla.Transform(self.waypoints[500], carla.Rotation(0,0,0))
        #actor = self.world.spawn_actor(victim_blueprint, spawn_point)
        ...
        #self.attackers = [PositionAttacker(self.waypoints[1500])]# [TrafficJamAttack(self.waypoints[10], 6)]
        prev_denm = {'Protocol Version': 1, 'Message ID': 1, 'Station ID': 167, 'Originating Station ID': 167, 'Sequence Number': 1, 'Altitude [Confidence]': ' 800001[15]', 'Longitude': -55.489256, 'Latitude': -0.003048, 'Semi Major Orientation': 3601, 'Semi Major Confidence': 4095, 'Semi Minor Confidence': 4095, 'Relevance Distance': 1, 'Relevance Traffic Detection': 0, 'Validity Duration': 600, 'Speed': 0.0, 'Speed Confidence': ' 3', 'Heading [Confidence]': ' 2189[10]', 'Information Quality': 0, 'Cause Code': 6, 'Sub Cause Code': 0, 'Alitude [Confidence]': 15.0, 'Speed_Confidence': 3.0, 'Heading': 2189.0, 'Heading_Confidence': 10.0, 'sender_pos_x': -57.16147994995117, 'sender_pos_y': 340.4968566894531}
        
        self.attackers=[TrafficJamAttack(prev_denm, -57.16147994995117, 340.4968566894531)]
    ...
    def run_artery(self,path):
        if path !="":
            ...
            p = Popen(['sleep 2 ;'+path+'/run_artery.sh sumo-omnetpp-med.ini', '--port', "65432"], cwd='./carla/Co-Simulation/arterysim/', shell=True)
        else:
            p = Popen(['sleep 2 ;'+os.path.expanduser('~')+'/artery/build/run_artery.sh sumo-omnetpp-med.ini', 'p'], cwd='./carla/Co-Simulation/arterysim/', shell=True)
        return p
    ...
    def step(self):
        ...
        if  self.artery_conn.is_connected() :
            current_step_cams= self.artery_conn.recieve_denm_messages(self.synchronization)# self.artery_conn.recieve_cam_messages(self.synchronization)
            # New injection
            attacker_cams = []
            for attacker in self.attackers:
                if np.random.random() < 1 and len(current_step_cams) > 0:
                    attacker_cams += attacker.perform_attack()
            current_step_cams += attacker_cams
            ...
            if self.args.color_cams:
                print("Attempting to color CAMs")
                #[self.carla_painter.color_communication(self.synchronization,cam) for cam in current_step_cams]
                #[self.carla_painter.color_communication(self.synchronization,cam) for cam in attacker_cams]
                [self.carla_painter.colorDENMS(self.synchronization,denm) for denm in attacker_cams]
